...
public class IDVerifierUtil {
	 ...
	 * 每位加权因子
	 */
	private final int power[] = { 7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5,
			8, 4, 2 };
	 ...
	 * @date 2015年12月1日 下午4:12:11
	 * @author libo
	 */
	public static boolean isValidated(String idCard) {
		if (!StringUtil.isBlank(idCard)) {
			return new IDVerifierUtil().isValidatedAllIdcard(idCard);
		}
		return false;
	}
	 ...
	 * @param idcard
	 * @return
	 * @date 2015年12月1日 下午4:14:31
	 * @author libo
	 */
	private boolean isValidatedAllIdcard(String idCard) {
		if (idCard.length() == 15) {
	...
	private String convertIdcarBy15bit(String idcard) {
		...
		if (StringUtil.isDigital(idcard)) {
			// 获取出生年月日
	...
	/**
	 * <p>
	 * 身份证号的每位和对应位的加权因子相乘之后，再得到和值;
	 * </p>
	 * 将和值与11取模得到余数s
	 * 
	 * @param sum17
	 *            身份证号的每位和对应位的加权因子相乘之后，所得到的和值
	 * @return 校验位
	 * @date 2015年12月1日 下午4:27:15
	 * @author libo
	 */
