...
$(function() {
	...
	aTab = function na(target) {
		var t = $(target).attr("href"),
			a = $(target).data("index") === undefined ? $.trim(t).length+1 : $(target).data("index"),
			i = $.trim($(target).attr('data-tabTitle')),
			n = !0,
			isBe = false;
		if (void 0 == t || 0 == $.trim(t).length) return !1;
		if ($(parent.document).find(".J_menuTab").each(function(index, v) {
				if($(v).data('id') == t){
					$(v).remove();
					isBe = true;
				}
				 return $(target).data("id") == t ? ($(target).hasClass("active") || ($(target).addClass("active").siblings(".J_menuTab").removeClass("active"), e(target), $(parent.document).find(".J_mainContent .J_iframe").each(function(i, v) {
					return  $(target).data('id') == t ? ($(target).show().siblings(".J_iframe").hide(), !1) : void 0
				})), n = !1, !1) : void 0
			}), n) {
			...
			if(!isBe){
				...
				var r = '<iframe class="J_iframe" name="iframe' + a + '" width="100%" height="100%" src="' + t + '" frameborder="0" data-id="' + t + '" seamless></iframe>';
				$(parent.document).find('.J_mainContent').find("iframe.J_iframe").hide().parents(".J_mainContent").append(r);
				...
				$(parent.document).find('.J_mainContent iframe:visible').load(function() {
				 ...
				 $(parent.document).find(".J_mainContent .J_iframe").each(function(index, v) {
					 if($(v).data('id') == t){
						 $(parent.document).find(".J_menuTab").removeClass("active");
	...
	}), $(".J_menuTabs").on("click", ".J_menuTab i", s), $(".J_tabCloseOther").on("click", r), $(".J_tabShowActive").on("click", o), $(".J_menuTabs").on("click", ".J_menuTab", d), $(".J_menuTabs").on("dblclick", ".J_menuTab", c), $(".J_tabLeft").on("click", a), $(".J_tabRight").on("click", i), $(".J_tabCloseAll").on("click", function() {
